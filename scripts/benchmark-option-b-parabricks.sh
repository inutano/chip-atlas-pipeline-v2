#!/bin/bash
#
# Benchmark Option B + Parabricks (fastp + GPU) on ce11 samples.
# Uses cached FASTQs. Runs in parallel with CPU benchmarks (GPU only).
#
set +e

BASE_DIR="$HOME/repos/chip-atlas-pipeline-v2"
TEST_DIR="/data3/chip-atlas-v2/test-run"
SAMPLES_TSV="$BASE_DIR/data/validation-samples.tsv"
WORKFLOW="$BASE_DIR/cwl/workflows/option-b-parabricks.cwl"
TIMING_LOG="$BASE_DIR/data/benchmark-timing-option-b-parabricks.tsv"

TARGET_GENOME="ce11"
GENOME_SIZE="ce"

genome_dir="$TEST_DIR/$TARGET_GENOME"
bwa_dir="$genome_dir/bwa-index"
fa="$bwa_dir/${TARGET_GENOME}.fa"

if [ ! -f "${fa}.bwt" ]; then
  echo "ERROR: BWA index not found at $fa"
  exit 1
fi

if [ ! -f "$TIMING_LOG" ]; then
  printf "accession\tgenome\texperiment_type\tnum_reads\tdownload_sec\tpipeline_sec\ttotal_sec\ttimestamp\n" > "$TIMING_LOG"
fi

tail -n +2 "$SAMPLES_TSV" | awk -F'\t' -v g="$TARGET_GENOME" '$2==g' | while IFS=$'\t' read -r accession genome exp_type antigen cell_type cell_class title num_reads mapping_rate dup_rate num_peaks read_tier; do
  if grep -q "^${accession}	" "$TIMING_LOG" 2>/dev/null; then
    echo "[SKIP] $accession"
    continue
  fi

  echo "========================================"
  echo "Option B+GPU: $accession ($exp_type, ${num_reads} reads)"
  echo "========================================"

  cache_dir="$genome_dir/fastq-cache"
  work_dir="$genome_dir/work-optb-pb/$accession"
  output_dir="$genome_dir/results-optb-pb/$accession"
  mkdir -p "$cache_dir" "$work_dir" "$output_dir"

  srr=$(curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=sra&id=${accession}&rettype=xml" \
    | grep -oP '[DES]RR[0-9]{7,}' | sort -u | head -1)

  if [ -z "$srr" ]; then
    echo "[ERROR] Could not resolve SRR for $accession, skipping"
    continue
  fi

  dl_start=$(date +%s)
  bash "$BASE_DIR/scripts/fast-download.sh" "$srr" "$cache_dir" 2>&1 | tail -3
  dl_end=$(date +%s)
  dl_sec=$((dl_end - dl_start))

  if [ -f "${cache_dir}/${srr}_1.fastq" ]; then
    fwd="${cache_dir}/${srr}_1.fastq"
  elif [ -f "${cache_dir}/${srr}.fastq" ]; then
    fwd="${cache_dir}/${srr}.fastq"
  else
    fwd=$(ls "${cache_dir}/${srr}"*.fastq 2>/dev/null | head -1)
    if [ -z "$fwd" ]; then
      echo "[WARN] No FASTQ found for $srr, skipping"
      continue
    fi
  fi

  input_yml="$work_dir/input.yml"
  cat > "$input_yml" <<YAML
sample_id: ${accession}
fastq_fwd:
  class: File
  path: ${fwd}
YAML

  if [ -f "${cache_dir}/${srr}_2.fastq" ]; then
    cat >> "$input_yml" <<YAML
fastq_rev:
  class: File
  path: ${cache_dir}/${srr}_2.fastq
format: BAMPE
YAML
  else
    cat >> "$input_yml" <<YAML
format: BAM
YAML
  fi

  cat >> "$input_yml" <<YAML
genome_fasta:
  class: File
  path: ${fa}
  secondaryFiles:
    - class: File
      path: ${fa}.fai
    - class: File
      path: ${fa}.amb
    - class: File
      path: ${fa}.ann
    - class: File
      path: ${fa}.bwt
    - class: File
      path: ${fa}.pac
    - class: File
      path: ${fa}.sa
chrom_sizes:
  class: File
  path: ${genome_dir}/chrom.sizes
genome_size: "${GENOME_SIZE}"
num_gpus: 1
YAML

  echo "[PIPELINE] Running Option B + Parabricks..."
  pipe_start=$(date +%s)

  if cwltool --enable-ext --outdir "$output_dir" "$WORKFLOW" "$input_yml" 2>&1 | tee "$output_dir/cwltool.log" | tail -5; then
    pipe_end=$(date +%s)
    pipe_sec=$((pipe_end - pipe_start))
    total_sec=$((dl_sec + pipe_sec))
    printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
      "$accession" "$TARGET_GENOME" "$exp_type" "$num_reads" "$dl_sec" "$pipe_sec" "$total_sec" \
      "$(date -Iseconds)" >> "$TIMING_LOG"
    rm -f "$output_dir"/*.bam "$output_dir"/*.bam.bai
  else
    pipe_end=$(date +%s)
    pipe_sec=$((pipe_end - pipe_start))
    total_sec=$((dl_sec + pipe_sec))
    printf "%s\t%s\t%s\t%s\t%s\tFAILED\t%s\t%s\n" \
      "$accession" "$TARGET_GENOME" "$exp_type" "$num_reads" "$dl_sec" "$total_sec" \
      "$(date -Iseconds)" >> "$TIMING_LOG"
  fi

  rm -rf "$work_dir"
  echo "[DONE] $accession (dl ${dl_sec}s, pipe ${pipe_sec:-?}s)"
  echo ""
done

COMPLETED=$(tail -n +2 "$TIMING_LOG" | grep -cv 'FAILED' || true)
FAILED=$(tail -n +2 "$TIMING_LOG" | grep -c 'FAILED' || true)

echo "========================================"
echo "Option B + Parabricks benchmark complete!"
echo "Completed: $COMPLETED, Failed: $FAILED"
echo "========================================"

curl -s -X POST https://api.getmoshi.app/api/webhook \
  -H "Content-Type: application/json" \
  -d "{\"token\": \"ADEnnNHEbkI20RHRpBIvf0q2ufMe9orI\", \"title\": \"Option B+GPU\", \"message\": \"ce11 Option B+GPU done. ${COMPLETED} OK, ${FAILED} failed.\"}"
