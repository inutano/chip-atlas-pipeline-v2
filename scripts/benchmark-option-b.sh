#!/bin/bash
#
# Benchmark Option B (Modern) on ce11 samples.
# Uses cached FASTQs from previous runs.
#
set +e
...
WORKFLOW="$BASE_DIR/cwl/workflows/option-b.cwl"
TIMING_LOG="$BASE_DIR/data/benchmark-timing-option-b.tsv"
...
tail -n +2 "$SAMPLES_TSV" | awk -F'\t' -v g="$TARGET_GENOME" '$2==g' | while IFS=$'\t' read -r accession genome exp_type antigen cell_type cell_class title num_reads mapping_rate dup_rate num_peaks read_tier; do
  ...
  echo "========================================"
  echo "Option B: $accession ($exp_type, ${num_reads} reads)"
  echo "========================================"

  cache_dir="$genome_dir/fastq-cache"
  work_dir="$genome_dir/work-optb/$accession"
  output_dir="$genome_dir/results-optb/$accession"
...
echo "========================================"
echo "Option B benchmark complete!"
echo "Completed: $COMPLETED, Failed: $FAILED"
echo "========================================"

curl -s -X POST https://api.getmoshi.app/api/webhook \
  -H "Content-Type: application/json" \
  -d "{\"token\": \"ADEnnNHEbkI20RHRpBIvf0q2ufMe9orI\", \"title\": \"Option B Benchmark\", \"message\": \"ce11 Option B done. ${COMPLETED} OK, ${FAILED} failed.\"}"
